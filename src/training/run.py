...
import pandas as pd
import torch
from .preprocessers import xsum_preprocess_function, cnn_aligned_preprocess_function
...
def run_training(parser: HfArgumentParser):
    ...
    # Load dataset
    dataset = load_dataset(data_args.dataset_name, cache_dir=cache_dir, revision="c8d80f3b036f297c713e4e2ce86b123ccc9ae961")
    ...
    if data_args.dataset_name == 'EdinburghNLP/xsum' or data_args.dataset_name == 'lilferrit/xsum_t5_distillation':
        preprocess_function = xsum_preprocess_function
    elif data_args.dataset_name == 'lilferrit/cnn_dailymail_t5_distillation':
        preprocess_function = cnn_aligned_preprocess_function
    else:
    ...
    trainer.train()
    trainer.push_to_hub(commit_message=data_args.commit_message, tags="summarization")
